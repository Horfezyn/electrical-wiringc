...
// Auxiliar centralized conductor search function
...
// --- Functions for Calculation and Retrieval ---
...
// Suggested gauge with insulation type check
int get_suggested_gauge_awg_kcmil(float arg_adjusted_current_amps, const char *arg_insulation_type_ptr, int arg_temp_rating) {
...
// Temperature correction factor validation
float get_temp_correction_factor(int arg_ambient_temp){
...
// Number of conductor validation
float get_ncond_adj_factor(int arg_conductor_count){
    ...
    // If even no lower match is found
